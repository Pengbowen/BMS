...
public class IBatchAddSubClassAction extends ResultOperateAction{
		...
		try {
			...
			if(data!=null){
				...
				for(int i=0;i<items.size();i++){
					...
					SingleSSTSubClassify s = generateBean(belongItemId);
					s.setSubClassName(map.get("项目名称"));
					int j = serviceSubClassify.insert(s);
					if(j<=0){
	 ...
	 * @return
	 */
	private SingleSSTSubClassify generateBean(String belongItemId){
		...
		//验证非法登录
		 HttpServletRequest request = ServletActionContext.getRequest();
			...
			SingleSSTSubClassify x = new SingleSSTSubClassify();
			x.setSSTId(SSTId);
