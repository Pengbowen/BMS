...
<script type="text/javascript">       
	...
	Ext.onReady(function() {
				...
				var fp = new Ext.FormPanel(
						{
						    url: rootPath+"extjs/upload/uploadController.jsp?t="+ Math.random()+"&fileURL=uploadfiles/lawslibrary",
							renderTo : 'fi-form',
							fileUpload : true,
							width : 500,
							frame : true,
							...
							items : [  {
								...
								validator: function(value){
						         ...
						         var arr = value.split('.');
						         if(arr[arr.length-1] != 'doc' && arr[arr.length-1] != 'pdf' && arr[arr.length-1] != 'docx'){
						             return '文件不合法！！！';
						           }else{
						             return true;
						            }
	...
	function saveMessage() {
		...
		var url = "";
		var path="savedpath=" + $("#savedpath").val();
		var arrs = path.split('=');
		var savePath=arrs[arrs.length-1]; 
		window.parent.closeWin();
		window.parent.getPath(savePath);
	}
