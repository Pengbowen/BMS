...
<script type="text/javascript">
	...
	//调用状态设置方法,默认为启用
  	...
  	addFirstItem();
  	   ...
  	   //设置层级和显示顺序的方法
   		...
   		var data = datalist[0];
   		var layerNo = data.layerNo;
   		//var displayOrder = data.displayOrder;
   			 $("#layerNo1").textbox("setValue",layerNo);
   	//		 $("#displayOrder").textbox("setValue",displayOrder);
  		 }
  		 //如过体系表不具有从属关系,则隐藏父级项目下拉选框
  		 if(SSTType!="0"){
  		 	$('#select').combobox({    
    			...
    			valueField:'id',    
   		 		...
   		 		var url = rootPath+"sst/searchLayerNo.action";
   		 		var layerItemId = sel.id;
   		 		var data = "layerItemId="+layerItemId;
   		 		//选择事件,当选择时触发
   		 		AjaxJson(url, true, data, setData, true, false)
   ...
   //设置displayOrder
   function setDisplayOrder(newNo,oldNo){
   	if(newNo!=""){
   	var layerNo = newNo;
   	dataStr = "layerNo="+layerNo;
   	var url = rootPath +"sst/serachMacDisplayOrder.action";
   	AjaxJson(url, false, dataStr, function(datalist) {
   	
   		var data = datalist[0];
   		var disNo = data.displayOrder;
   		 $("#displayOrder").textbox("setValue",disNo);
   	}, true, false)
   	}
		...
		<form id="myForm" >
			...
			<tr>
				...
				<tr>
					...
					<td class="list-info"><input class="easyui-textbox" type="text" data-options="onChange:setDisplayOrder" style="width:200px;height:28px;" id="layerNo1" name="layerNo1" value="${layerNo1}"  />
						<font color="red" style="padding-right:0;">*必填</font>
					</td>
				</tr>
				<tr>
